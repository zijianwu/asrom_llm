from asrom_llm.clinical_reference import DocumentOutline, Section
...
def test_section_full_title():
    ...
    section2 = Section("Section 1.1", content="Section 1.1 content", prev_titles=prev_titles)
...
def test_section_str_representation():
    section_with_content = Section("Title with content", content="Some content")
    section_without_content = Section("Title without content")

    assert str(section_with_content) == "Title with content (Has content True)"
    assert str(section_without_content) == "Title without content (Has content False)"
...
def test_document_outline_find_section_by_title():
    ...
    heading_1 = doc_outline.find_section_by_title("Heading 1", doc_outline.root)
    subheading_1 = doc_outline.find_section_by_title("Subheading 1", doc_outline.root)
...
def test_document_outline_create_doc_outline_from_outline():
    ...
    doc_outline = DocumentOutline("Test TOC")
    doc_outline._create_toc_from_outline(outline)

    heading_1 = doc_outline.find_section_by_title("Heading 1", doc_outline.root)
    subheading_1 = doc_outline.find_section_by_title("Subheading 1", doc_outline.root)
    heading_2 = doc_outline.find_section_by_title("Heading 2", doc_outline.root)
    subheading_2 = doc_outline.find_section_by_title("Subheading 2", doc_outline.root)
